            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                try
                {
                    if (_perRequestLanguageStrings.ContainsKey(trimmedKey))
                    {
                        ...
                        //_loggingService.Error($"No value is set for resource key '{trimmedKey}' for language {CurrentLanguage.Name}.");
                    }
                    else
                    {
                        //_loggingService.Error($"This resource key '{trimmedKey}' was not found for the language {CurrentLanguage.Name}.");
                    }
                }
                catch (Exception ex)
                {
